...
---@return Theme
function lazord.setup()
  local config = require("Lazord.config").options
  local colors = require("Lazord.colors").setup(config)
  ...
  local function make_bold(group)
  ...
  local function make_sidebar_bg()
    if opts.styles.sidebars == "transparent" then
      return c.none
    elseif opts.styles.sidebars == "dark" then
      return c.bg_dark
    else
      return c.bg
    end
  ...
  local function make_float_bg()
    if opts.styles.floats == "transparent" then
      return c.none
    elseif opts.styles.floats == "dark" then
      return c.bg_dark
    else
      return c.bg
    end
  ...
  theme.highlights = {
    ...
    Comment                                  = { fg = c.fg2, style = opts.styles.comments }, -- any comment
    ...
    CursorLine                               = { bg = opts.transparent and c.bg0 or c.bg1 }, -- Screen-line at the cursor, when 'cursorline' is set.  Low-priority if foreground (ctermfg OR guifg) is not set.
    ...
    ErrorText                                = { sp = c.diagError, undercurl = opts.styles.undercurl },
    FloatBorder                              = { fg = c.base1, bg = opts.transparent and c.none or c.bg1 },
    FoldColumn                               = { bg = opts.transparent and c.none or c.bg0, fg = c.bg1 }, -- 'foldcolumn'
    Folded                                   = { fg = c.blue, bg = c.bg1 },                                  -- line used for closed folds
    Foo                                      = { bg = c.purple, fg = c.purple },
    HintText                                 = { sp = c.diagHint, undercurl = false },
    InfoText                                 = { sp = c.diagInfo, undercurl = opts.styles.undercurl },
    IncSearch                                = { bg = c.orange, fg = c.bg0 },                                     -- 'incsearch' highlighting; also used for the text replaced with ":s///c"
    LineNr                                   = { bg = opts.transparent and c.none or c.bg1, fg = c.fg1 },      -- Line number for ":number" and ":#" commands, and when 'number' or 'relativenumber' option is set.
    ...
    Normal                                   = { fg = c.fg0, bg = opts.transparent and c.none or c.bg0 },      -- normal text
    NormalFloat                              = { fg = c.fg0, bg = opts.transparent and c.none or c.bg1 },      -- Normal text in floating windows.
    NormalNC                                 = { fg = c.fg0, bg = opts.transparent and c.none or c.bg0 },      -- normal text in non-current windows
    NormalSB                                 = { fg = c.fg0, bg = opts.transparent and c.none or c.bg0 },       -- normal text in sidebar
    Pmenu                                    = { bg = opts.transparent and c.none or c.bg1, fg = c.fg0 },      -- Popup menu: normal item.
    PmenuSbar                                = { bg = opts.transparent and c.none or c.bg1 },                  -- Popup menu: scrollbar.
    PmenuSel                                 = { bg = c.fg2, fg = c.base3 },                                      -- Popup menu: selected item.
    PmenuThumb                               = { bg = c.base1 },                                                  -- Popup menu: Thumb of the scrollbar.
    Question                                 = { fg = c.blue },                                                   -- |hit-enter| prompt and yes/no questions
    QuickFixLine                             = { bg = c.bg1, bold = true, undercurl = opts.styles.undercurl }, -- Current |quickfix| item in the quickfix window. Combined with |hl-CursorLine| when the cursor is there.
    ...
    SignColumn                               = { bg = opts.transparent and c.none or c.bg0 },                  -- column where |signs| are displayed
    SignColumnSB                             = { bg = opts.transparent and c.none or c.bg0, fg = c.bg1 },        -- column where |signs| are displayed
    ...
    SpellBad                                 = { sp = c.red, undercurl = opts.styles.undercurl },              -- Word that is not recognized by the spellchecker. |spell| Combined with the highlighting used otherwise.
    SpellCap                                 = { sp = c.blue, undercurl = opts.styles.undercurl },             -- Word that should start with a capital. |spell| Combined with the highlighting used otherwise.
    SpellLocal                               = { sp = c.purple, undercurl = opts.styles.undercurl },           -- Word that is recognized by the spellchecker as one that is used in another region. |spell| Combined with the highlighting used otherwise.
    SpellRare                                = { sp = c.green, undercurl = opts.styles.undercurl },            -- Word that is recognized by the spellchecker as one that is hardly ever used.  |spell| Combined with the highlighting used otherwise.
    StatusLine                               = { fg = c.fg0, bg = opts.transparent and c.none or c.bg0 },      -- status line of current window
    ...
    TabLineFill                              = { bg = opts.transparent and c.none or c.bg0 },                    -- tab pages line, where there are no labels
    TabLineSel                               = { fg = c.bg0, bg = c.blue },                                       -- tab pages line, active tab page label
    Title                                    = { fg = c.blue, bold = true },                                      -- titles for output from ":set all", ":autocmd" etc.
    VertSplit                                = { fg = opts.transparent and c.bg0 or c.base1 },                 -- the column separating vertically split windows
    Visual                                   = { bg = c.bg1, fg = c.none },                                       -- Visual mode selection
    VisualNOS                                = { bg = c.bg1 },                                               -- Visual mode selection when vim is "Not Owning the Selection".
    WarningMsg                               = { fg = c.red },                                               -- warning messages
    WarningText                              = { sp = c.diagWarn, undercurl = opts.styles.undercurl },
    Whitespace                               = { fg = c.fg2 },                                               -- "nbsp", "space", "tab" and "trail" in 'listchars'
    WildMenu                                 = { bg = c.bg1 },                                               -- current match in 'wildmenu' completion
    WinSeparator                             = { fg = opts.transparent and c.bg0 or c.bg1, bold = true }, -- the column separating vertically split windows
    ...
    String                                   = { fg = c.green, style = opts.styles.string },     --   a string constant: "this is a string"
    ...
    Identifier                               = { fg = c.blue, style = opts.styles.variables },   -- (preferred) any variable name
    Function                                 = { fg = c.blue, style = opts.styles.functions },   -- function name: #2176c7
    Statement                                = { fg = c.red, italic = opts.enable_italics },     -- (preferred) any statement
    Conditional                              = { fg = c.aqua, italic = opts.enable_italics },    --  if, then, else, endif, switch, etc.
    Repeat                                   = { fg = c.red, italic = opts.enable_italics },     --   for, do, while, etc.
    Label                                    = { fg = c.orange },                                   --    case, default, etc.
    Operator                                 = { fg = c.orange },                                   -- "sizeof", "+", "*", etc.
    Keyword                                  = { fg = c.red, style = opts.styles.keywords },     --  any other keyword
    Exception                                = { fg = c.red, italic = opts.enable_italics },     --  try, catch, throw
    PreProc                                  = { fg = c.purple, italic = opts.enable_italics },  -- (preferred) generic Preprocessor
    PreCondit                                = { fg = c.purple, italic = opts.enable_italics },
    Include                                  = { fg = c.red, italic = opts.enable_italics },     --  preprocessor #include
    Define                                   = { fg = c.purple, italic = opts.enable_italics },  --   preprocessor #define
    ...
    Typedef                                  = { fg = c.red, italic = opts.enable_italics },     --  A typedef
    ...
    Delimiter                                = { fg = c.green },                                     -- Commas, semicolons and other delimiters
    ...
    -- Diagnostic
    RedSign                                  = { fg = c.diagError },
    YellowSign                               = { fg = c.diagWarn },
    GreenSign                                = { fg = c.diagHint },
    BlueSign                                 = { fg = c.diagInfo },
    VirtualTextWarning                       = { fg = c.diagWarn },
    VirtualTextError                         = { fg = c.diagError },
    VirtualTextInfo                          = { fg = c.diagInfo },
    VirtualTextHint                          = { fg = c.diagHint },
    ErrorFloat                               = { fg = c.diagError, bg = opts.transparent and c.none or c.bg1 },
    WarningFloat                             = { fg = c.diagWarn, bg = opts.transparent and c.none or c.bg1 },
    InfoFloat                                = { fg = c.diagInfo, bg = opts.transparent and c.none or c.bg1 },
    HintFloat                                = { fg = c.diagHint, bg = opts.transparent and c.none or c.bg1 },
    --[[ WinBarNC                   = { fg = c.base1 }, ]]
    ...
    -- Additional diagnostic groups for better LSP compatibility
    ...
    LspCodeLens                              = { fg = c.magenta },                -- Code lenses: #c61c6f
    LspCodeLensSeparator                     = { fg = c.cyan },                  -- Code lens separators: #259286
    ...
    TSField                                  = { fg = c.green },
    TSFloat                                  = { fg = c.purple },
    TSFuncBuiltin                            = { fg = c.cyan, bold = true },     -- Built-in functions: #259286
    TSFuncMacro                              = { fg = c.magenta, bold = true }, -- Function macros: #c61c6f
    TSFunction                               = { fg = c.blue, bold = true },    -- Functions: #2176c7
    TSInclude                                = { fg = c.red },
    TSKeyword                                = { fg = c.red },
    TSKeywordFunction                        = { fg = c.red },
    TSKeywordOperator                        = { fg = c.orange },
    TSLabel                                  = { fg = c.orange },
    TSMethod                                 = { fg = c.green, bold = true },    -- Methods: #738a05
    ...
    TSPunctBracket                           = { fg = c.blue },    -- Default brackets (parentheses)
    TSPunctDelimiter                         = { fg = c.green },  -- Commas, semicolons
    TSPunctSpecial                           = { fg = c.orange }, -- Special punctuation
    TSRepeat                                 = { fg = c.red },
    ...
    -- Punctuation with specific colors for different bracket types
    ["@punctuation.delimiter"]               = { fg = c.green },        -- Commas: ,
    ["@punctuation.bracket"]                 = { fg = c.blue },         -- Parentheses: ()
    ["@punctuation.bracket.square"]          = { fg = c.red },          -- Square brackets: []
    ["@punctuation.bracket.curly"]           = { fg = c.yellow },       -- Curly braces: {}
    ["@punctuation.special"]                 = { fg = c.orange },       -- Special punctuation
    
    -- Legacy Tree-sitter bracket groups (for compatibility)
    ["@bracket"]                             = { fg = c.blue },         -- ()
    ["@bracket.square"]                      = { fg = c.red },          -- []
    ["@bracket.curly"]                       = { fg = c.yellow },       -- {}
    ["@delimiter"]                           = { fg = c.green },        -- ,
    
    -- Function and method related elements around curly braces
    ["@function"]                            = { fg = c.blue },         -- Functions: #2176c7
    ["@function.builtin"]                    = { fg = c.cyan },         -- Built-in functions: #259286
    ["@function.macro"]                      = { fg = c.magenta },      -- Function macros: #c61c6f
    ["@function.call"]                       = { fg = c.blue },         -- Function calls: #2176c7
    ["@method"]                              = { fg = c.green },        -- Methods: #738a05
    ["@method.call"]                         = { fg = c.green },        -- Method calls: #738a05
    
    -- LSP semantic tokens for functions
    ["@lsp.type.function"]                   = { fg = c.blue },         -- LSP functions: #2176c7
    ["@lsp.type.method"]                     = { fg = c.green },        -- LSP methods: #738a05
    ["@lsp.mod.defaultLibrary"]              = { fg = c.cyan },         -- LSP built-in functions: #259286
    
    -- Additional function-related groups that might be missing
    ["@constructor"]                         = { fg = c.blue },         -- Constructor functions: #2176c7
    ["@function.method"]                     = { fg = c.green },        -- Method functions: #738a05
    ["@keyword.function"]                    = { fg = c.magenta },      -- Function keywords: #c61c6f
    ...
    TelescopeBorder                          = { fg = c.base1, bg = opts.transparent and c.none or c.bg0 },
    TelescopeNormal                          = { fg = c.fg0, bg = opts.transparent and c.none or c.bg0 },
    ...
    NeoTreeNormal                            = { fg = c.fg0, bg = opts.transparent and c.none or c.bg0 },
    ...
    NeoTreeTabActive                         = { bg = opts.transparent and c.none or c.bg0, fg = c.aqua, bold = true },
    NeoTreeTabInactive                       = { bg = opts.transparent and c.none or c.bg1, fg = c.base2 },
    NeoTreeTabSeparatorActive                = {
      fg = opts.transparent and c.none or c.bg0,
      bg = opts.transparent and c.none or c.bg0
    },
    NeoTreeTabSeparatorInactive              = {
      fg = opts.transparent and c.none or c.bg1,
      bg = opts.transparent and c.none or c.bg1
    },
    NeoTreeVertSplit                         = { fg = c.base, bg = opts.transparent and c.none or c.bg1 },
    ...
    WhichKeyFloat                            = { bg = opts.transparent and c.none or c.bg1 },
    ...
    -- Cmp
    CmpDocumentation                         = { fg = c.fg0, bg = opts.transparent and c.none or c.bg1 },
    CmpDocumentationBorder                   = { fg = c.bg1, bg = opts.transparent and c.none or c.bg0 },
    ...
    -- Indent Blank Line (modern indent-blankline.nvim v3+)
    IblIndent                                = { fg = c.dark3, bg = c.none },
    IblScope                                 = { fg = c.blue, bg = c.none },                      -- Scope lines: #2176c7
    IblWhitespace                            = { fg = c.dark3, bg = c.none },
    -- Legacy indent-blankline.nvim (v2) groups
    IndentBlanklineChar                      = { fg = c.dark3, bg = c.none },
    IndentBlanklineContextChar               = { fg = c.blue, bg = c.none },                      -- Context: #2176c7
    IndentBlanklineSpaceChar                 = { fg = c.dark3, bg = c.none },
    IndentBlanklineSpaceCharBlankline        = { fg = c.dark3, bg = c.none },
    ...
    MiniIndentscopeSymbol                    = { fg = opts.transparent and c.bg1 or c.fg2 },
    ...
    MiniStarterInactive                      = { fg = c.base1, style = opts.styles.comments },
    MiniStarterItem                          = { fg = c.fg0, bg = opts.transparent and c.none or c.bg0 },
    ...
    cssBraces                                 = { fg = c.fg0 },
    cssTagName                                = { fg = c.purple, italic = opts.enable_italics },
    cssClassNameDot                           = { fg = c.red },
    cssClassName                              = { fg = c.red, italic = opts.enable_italics },
    ...
    cssImportant                              = { fg = c.red, italic = opts.enable_italics },
    ...
    javaScriptFunction                        = { fg = c.red, italic = opts.enable_italics },
    ...
    -- Lua
    luaFunc                                   = { fg = c.blue, style = opts.styles.bold },    -- Lua functions: #2176c7
    luaFunction                               = { fg = c.blue },                               -- Lua function keyword: #2176c7
    luaTable                                  = { fg = c.fg0 },
    luaIn                                     = { fg = c.red, italic = opts.enable_italics },
    -- Go
    goDirective                               = { fg = c.purple, italic = opts.enable_italics },
    goConstants                               = { fg = c.aqua },
    goDeclType                                = { fg = c.orange, italic = opts.enable_italics },
    -- Rust
    ...
    rustSelf                                  = { fg = c.blue },
    rustSuper                                 = { fg = c.blue },
    rustDeriveTrait                           = { fg = c.purple, italic = opts.enable_italics },
    rustEnumVariant                           = { fg = c.purple },
    rustMacroVariable                         = { fg = c.blue },
    rustAssert                                = { fg = c.aqua },
    rustPanic                                 = { fg = c.aqua },
    rustPubScopeCrate                         = { fg = c.purple, italic = opts.enable_italics },
    -- PHP
    phpVarSelector                            = { fg = c.blue },
    phpDefine                                 = { fg = c.orange, italic = opts.enable_italics },
    phpStructure                              = { fg = c.red, italic = opts.enable_italics },
    ...
    -- Java
    javaClassDecl                             = { fg = c.red, italic = opts.enable_italics },
    javaMethodDecl                            = { fg = c.red, italic = opts.enable_italics },
    ...
    rubyDefine                                = { fg = c.red, italic = opts.enable_italics },
    rubyModuleName                            = { fg = c.purple },
    rubyAccess                                = { fg = c.orange },
    rubyAttribute                             = { fg = c.yellow },
    rubyMacro                                 = { fg = c.red, italic = opts.enable_italics },
    -- sh/zsh
    shRange                                   = { fg = c.fg0 },
    ...
    shFunctionKey                             = { fg = c.red, italic = opts.enable_italics },
    -- ZSH
    zshOptStart                               = { fg = c.purple, italic = opts.enable_italics },
    zshOption                                 = { fg = c.blue },
    ...
    vimMap                                    = { fg = c.purple, italic = opts.enable_italics },
  ...
  opts.on_highlights(theme.highlights, theme.colors)
